...
module.exports = async function(req, res, instance) {
    ...
    let postModifyGupImpuResponse = instance.postModifyGupImpuResponse;
    instance.currentState = STATE.W_POST_MODIFYIDENTITY_GUPIMPU;
    let data;
    let resultCode;
    let resultDesc;
    let errMessageStacks;

    if(validator.isDefinedValue(postModifyGupImpuResponse)){
        data = postModifyGupImpuResponse.data;
        if(validator.isDefinedValue(data)){
            resultCode = data.resultCode;
            resultDesc = data.resultDescription;
            errMessageStacks = data.errorMessageStack;
        }else{
            resultCode = postModifyGupImpuResponse.statusCode;
            resultDesc = postModifyGupImpuResponse.statusMessage;
        }
    }
    ...
    try{
        sdfValidate.validatePostModifyIdentityResponse(postModifyGupImpuResponse);
    ...
    await utils.postGupImpu.call(this, instance);
    return;
