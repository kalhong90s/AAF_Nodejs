...
module.exports = async function(req, res, instance) {
    ...
    let postModifyGupImpiResponse = instance.postModifyGupImpiResponse;
    instance.currentState = STATE.W_POST_MODIFYIDENTITY_GUPIMPI;
    let data;
    let resultCode;
    let resultDesc;
    let errMessageStacks;

    if(validator.isDefinedValue(postModifyGupImpiResponse)){
        data = postModifyGupImpiResponse.data;
        if(validator.isDefinedValue(data)){
            resultCode = data.resultCode;
            resultDesc = data.resultDescription;
            errMessageStacks = data.errorMessageStack;
        }else{
            resultCode = postModifyGupImpiResponse.statusCode;
            resultDesc = postModifyGupImpiResponse.statusMessage;
        }
    }
    ...
    try{
        sdfValidate.validatePostModifyIdentityResponse(postModifyGupImpiResponse);
    ...
    await utils.postGupImpi.call(this, instance);
    return;
