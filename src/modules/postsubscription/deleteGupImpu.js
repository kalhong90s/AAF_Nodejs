...
module.exports = async function(req, res, instance) {
    ...
    let deleteGupImpuResponse = instance.deleteGupImpuResponse;
    instance.currentState = STATE.W_DELETE_GUPIMPU;
    let data;

    if(validator.isDefinedValue(deleteGupImpuResponse)){
        data = deleteGupImpuResponse.data;
    ...
    try{
        sdfValidate.validateDeleteGupImpuResponse(deleteGupImpuResponse);
        LOG.STAT.call(this, STAT.AAF_SDF_DELETE_GUP_IMPU_RESPONSE);
    }catch(error){
        LOG.DEBUG.DEBUG.call(this, instance, `Error : ${error.message}`);
        if(error.event === EXCEPTIONEVENT.MISSING_OR_INVALID_PARAMETER){
            LOG.STAT.call(this, STAT.AAF_SDF_DELETE_GUP_IMPU_RECEIVED_BAD);
            return;
        }else if(error.event === EXCEPTIONEVENT.DATA_NOT_FOUND){
            LOG.STAT.call(this, STAT.AAF_SDF_DELETE_GUP_IMPU_RECEIVED_ERROR);
            ...
            LOG.STAT.call(this, STAT.AAF_SDF_DELETE_GUP_IMPU_RECEIVED_TIMEOUT);
            return;
        }else if(error.event === EXCEPTIONEVENT.CONNECTION_ERROR){
            LOG.STAT.call(this, STAT.AAF_SDF_DELETE_GUP_IMPU_RECEIVED_CONNECTION_ERROR);
            return;
        }else{
            LOG.STAT.call(this, STAT.AAF_SDF_DELETE_GUP_IMPU_RECEIVED_ERROR);
            return;
