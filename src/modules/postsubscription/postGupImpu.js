...
module.exports = async function(req, res, instance) {
    ...
    let commandName = COMMAND.POST_GUPIMPU;

    let postGupImpuResponse = instance.postGupImpuResponse;
    instance.currentState = STATE.W_POST_GUPIMPU;
    let data;
    let resultCode;
    let resultDesc;
    let errMessageStacks;

    if(validator.isDefinedValue(postGupImpuResponse)){
        data = postGupImpuResponse.data;
        if(validator.isDefinedValue(data)){
            resultCode = data.resultCode;
            resultDesc = data.resultDescription;
            errMessageStacks = data.errorMessageStack;
        }else{
            resultCode = postGupImpuResponse.statusCode;
            resultDesc = postGupImpuResponse.statusMessage;
        }
    }
    ...
    try{
        sdfValidate.validatePostGupImpuResponse(postGupImpuResponse);
        LOG.STAT.call(this, STAT.AAF_SDF_POST_GUP_IMPU_RESPONSE);
    }catch(error){
        ...
        if(error.event === EXCEPTIONEVENT.MISSING_OR_INVALID_PARAMETER){
            ...
            LOG.STAT.call(this, STAT.AAF_SDF_POST_GUP_IMPU_RECEIVED_BAD);
            ...
            LOG.SUMMARYLOG.ERROR(instance, nodeName, commandName, resultCode, resultDesc);
            LOG.STAT.call(this, STAT.AAF_SDF_POST_GUP_IMPU_RECEIVED_ERROR);
            ...
            LOG.SUMMARYLOG.ERROR(instance, nodeName, commandName, resultCode, resultDesc);
            LOG.STAT.call(this, STAT.AAF_SDF_POST_GUP_IMPU_RECEIVED_TIMEOUT);
            ...
            LOG.SUMMARYLOG.ERROR(instance, nodeName, commandName, resultCode, resultDesc);
            LOG.STAT.call(this, STAT.AAF_SDF_POST_GUP_IMPU_RECEIVED_CONNECTION_ERROR);
